...
class Transformer(nn.Module):
    ...
    def __init__(self, input_size, key_size, hidden_size, output_size, initialization,
                num_layers, use_bias=True, bidirectional=True, attn=None, 
                transformer_order=None, attn_key=None, attn_dmetric=None, dropout=0, **kwargs):
        ...
        self.attn_dmetric = attn_dmetric

        # Intermediate architecure
        if num_layers > 0:
            for layer in range(num_layers):
                layer_input_size = 2*input_size+key_size if layer == 0 else hidden_size
        ...
        if transformer_order == 'zero':
            self.transformer_limits = None
        elif transformer_order == 'one':
            self.transformer_limits = (-2,0)
        elif transformer_order == 'two':
            self.transformer_limits = (-3,0) 
        elif transformer_order == 'inf':
            self.transformer_limits = (np.NINF,0)
        elif transformer_order == 'all':  
        ...
        for node_index, each_node in enumerate(lattice.nodes):
            ...
            if self.transformer_limits is not None:
                ...
                if each_node in lattice.child_dict:
                    ...
                    print(f'INTO ATTENTION {in_edges}')
                    ...
                    print(f'OUT ATTENTION {weighted_in_edges}')
